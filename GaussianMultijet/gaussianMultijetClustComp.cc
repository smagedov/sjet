...
int main(int argc, char *argv[])
{
    ...
    try {
        ...
        cmdline >> pt0 >> mult0 >> pt1 >> mult1 >> pt2 >> mult2 >> probeJetWidth
                >> deltaR >> nEvents >> nJets >> distanceCutoff >> outputFile;
    ...
    {
        const MultijetMaker<MyEvent,MyRng> MultijetGenerator(
            "MultijetMaker", gen, refJetWidth, pt0, mult0,
            probeJetWidth, pt1, mult1, pileupEtaRange, pt2, mult2,
            deltaR, ptMin, randomizeProbeJet, randomizeMult, nJets);
        mySequence.add(MultijetGenerator);
    }
    if (countPeriod)
    ...
    mySequence.add(FullJetHistoryPrinter<MyEvent>("0", "multijetHistFile", 1000, distanceCutoff));
