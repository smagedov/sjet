...
static void print_usage(const char* progname)
{
    std::cout << "\nUsage: " << progname
              ...
              << " [--refJetWidth value]"
              << " [--seed value]"
              << " avePtRef muRef avePtPileup muPileup nEvents distanceCutoff outputPrefix\n"
              << std::endl
              ...
              << std::endl
              << " outputPrefix      The names of the files into which jet histories will be\n"
              << "                   written will look like outputPrefix_N_J.txt, where N is\n"
              << "                   the event number and J is the jet number.\n"
              << std::endl;
}
...
int main(int argc, char *argv[])
{
    ...
    std::string dumpFile, outputPrefix;
    ...
    try {
        ...
        cmdline >> pt0 >> mult0 >> pt2 >> mult2
                >> nEvents >> distanceCutoff >> outputPrefix;
    ...
    mySequence.add(JetHistoryPrinter<MyEvent>(
                       "JetHistoryPrinter", outputPrefix, 1U));
    // mySequence.add(CalcAndPrint<MyEvent>("CalcAndPrint", outputFile));
