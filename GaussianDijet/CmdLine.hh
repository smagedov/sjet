...
public:
    ...
    inline CmdLineError(const char* msg)
        : os_(new std::ostringstream()) {if (msg) *os_ << msg;}

    inline CmdLineError(const std::string& msg)
        : os_(new std::ostringstream()) {if (!msg.empty()) *os_ << msg;}
...
#ifdef __GNUC__
    template <typename T>
    inline std::string demangle(const T& obj) const
    {
        int status;
